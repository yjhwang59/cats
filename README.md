...
push 到 `main` 分支時，由 **GitHub Actions** 自動執行 D1 migration（可選）並部署至 Cloudflare Pages（專案名稱 `cat-git`）。
...
**Workers & Pages** → **Create application** → **Pages** → **直接上傳（Direct Upload）**，專案名稱填 **`cat-git`**，建立後無需連接 Git（由 GitHub Actions 部署）。再於該專案 **Settings → Bindings** 新增 D1 綁定 `DB` → `catsd1_db`。
